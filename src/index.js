...
async function run() {
  try {
    ...
    const json = await request(appId, tokenString, platform);
...
function setOutput(data, index) {
  ...
  if (attributes == null) {
    throw new Error(
      `Something went wrong. Couldn't retrieve details for the ${type} app release...`
    );
  }
  ...
  console.log(
    `The ${type} App Store application release type is ${releaseType}`
  );
  console.log(
    `The ${type} App Store application release creation date is ${createdDate}`
  );
